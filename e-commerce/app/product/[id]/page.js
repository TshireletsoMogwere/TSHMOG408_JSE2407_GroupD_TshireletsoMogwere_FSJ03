      ...
      try {
          ...
          if (response.ok) {
              setProduct((prevProduct) => ({
                  ...prevProduct,
                  reviews: [...prevProduct.reviews, { ...data, reviewerName: 'Your Name' }],
              }));
              alert(data.message);
              setNewRating(0);
              setNewComment('');
          ...
          if (response.ok) {
              setProduct((prevProduct) => {
                  const updatedReviews = prevProduct.reviews.map((review) =>
                      review.id === reviewId ? { ...review, rating: newRating, comment: newComment } : review
                  );
      ...
      <div className="mt-5 ml-10 w-14">
            ...
            <div className="max-w-6xl mx-auto p-10">
                ...
                <div className="mt-10">
                    ...
                    {/* Review Form */}
                    <div className="mt-6">
                        <h2 className="text-xl font-semibold mb-4">{isEditing ? 'Edit Review' : 'Add a Review'}</h2>
                        <form onSubmit={isEditing ? (e) => { e.preventDefault(); handleEditReview(editingReviewId); } : handleAddReview}>
                        ...
                        <div className="mt-4">
                            {sortedReviews.map((review) => (
                                <div key={review.id} className="border-b border-gray-300 mb-4 pb-4">
                                    <div className="flex items-center mb-2">
                                        {[...Array(5)].map((_, i) => (
                                            <FaStar key={i} className={`w-5 h-5 ${i < review.rating ? 'text-orange-400' : 'text-gray-300'}`} />
                                        ))}
